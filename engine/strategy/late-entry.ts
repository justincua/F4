...
function readTradeShares(): number {
  const raw = process.env.TRADE_SHARES ?? process.env.ORDER_SHARES ?? process.env.SHARES ?? "6";
  const n = Number(raw);
  if (!Number.isFinite(n) || n <= 0) return 6;
  return Math.floor(n);
}

const SHARES = readTradeShares();
...
export const lateEntry: Strategy = async (ctx) => {
  ...
  if (Env.get("PROD") && process.env.ALLOW_LATE_ENTRY_PROD !== "true") {
    ctx.log(
      "[late-entry] Production bị khóa cho strategy này. " +
        "Nếu bạn đã test kỹ và chấp nhận rủi ro, set ALLOW_LATE_ENTRY_PROD=true.",
      "red",
    );
    process.exit(1);
  }
